#!/usr/bin/env python3
"""Compare current perf_profile metrics against a saved baseline."""
...
LINE_RE = re.compile(
    r"^(?P<name>.+?)\s+"
    r"(?P<throughput>[0-9]+)\s+"
    r"(?P<throughput_unit>msg/s|ev/s|snaps/s)\s+"
    r"(?P<ns>[0-9]+(?:\.[0-9]+)?)\s+"
    r"(?P<ns_unit>ns/msg|ns/ev|ns/snap)"
)
...
def run_perf_profile(command: List[str], cwd: Path) -> Dict[str, Dict[str, float | str]]:
    proc = subprocess.run(
        command,
        cwd=str(cwd),
        check=True,
    ...
    if not parsed:
        raise RuntimeError("No metrics parsed from perf_profile output.")
    return parsed
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="How many perf_profile runs to aggregate (median). Default: 3",
    )
    parser.add_argument(
        "--allowed-slowdown-pct",
        type=float,
        default=10.0,
        help="Allowed slowdown in percent per metric (based on ns/unit). Default: 10.0",
    )
    parser.add_argument(
        "--workdir",
        type=Path,
    ...
    command = ["cargo", "run", "--release", "--bin", "perf_profile"]
    runs = []
    for idx in range(args.runs):
        print(f"[check] run {idx + 1}/{args.runs}")
        runs.append(run_perf_profile(command, args.workdir))
